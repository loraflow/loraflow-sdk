...
#define PANIC FATALF //(...)   do { std::terminate(); } while (0)
...
#define ERRCODE(c)  Errcode{__SHORT_FILE__, __LINE__, c}

    using BoolResult = Result<bool,Errcode>;
    using IntResult = Result<int,Errcode>;
    using VoidResult = Result<void,Errcode>;
}

#endif //NEW_BACKHAUL_RESULT_H
