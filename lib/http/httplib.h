...
//        SocketStream strm;
    ...
    void write_response(socket_t sock,int len);
    bool is_running() const;
    void stop();
    socket_t getCurrentSock();

protected:
    bool process_request(Stream& strm, bool last_connection, bool& connection_close);
    void setCountPara(Stream& strm, bool last_connection, const Request& req, Response& res);
...
inline socket_t Server::getCurrentSock()
{
    return send_sock_ ;
}
...
inline void Server::write_response(socket_t sock,int len) {
...
void Server::setCountPara(Stream& strm, bool last_connection, const Request& req, Response& res) {
//    para.strm = strm;
//    para.last_connection = last_connection;
//    para.req = req;
//    para.res = res;
}
...
inline bool Server::process_request(Stream& strm, bool last_connection, bool& connection_close)
{
    ...
    if (req.method == "POST" || req.method == "PUT") {
        INFOF("正在发送回复..."); //在这里阻塞等待
        setCountPara(strm, last_connection, req, res);
        if (!detail::read_content(strm, req, Progress())) {
            res.status = 400;
            write_response(strm, last_connection, req, res);
            return ret;
        }
        ...
        INFOF("AAAAAAA type:{}",content_type);
